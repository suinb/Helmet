...
path = 'D:\HandGestureClassify-master\handGesturePic'
# 模型保存地址
model_path='D:\HandGestureClassify-master\classify\modelSave\model.ckpt'
...
def inference(input_tensor, train, regularizer):  # regularizer = tf.contrib.layers.l2_regularizer(0.0001)
    ...
    with tf.variable_scope('layer11-fc3'):
        fc3_weights = tf.get_variable("weight", [512, 20],
                                      initializer=tf.truncated_normal_initializer(stddev=0.1))
        if regularizer != None: tf.add_to_collection('losses', regularizer(fc3_weights))
        fc3_biases = tf.get_variable("bias", [20], initializer=tf.constant_initializer(0.1))
